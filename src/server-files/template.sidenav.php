                ...
                if($lev2->getValue('cat_nav_target') != '0'){
                    ...
                    foreach($referenceCategory->getArticles() as $navArticle){ 
                        ...
                        if(!$navArticle->isStartArticle()){
                            ...
                            $side_nav_sub_list .= '<li><a href="'.$articleUrl.'#'.$navArticleId.'"><i class="material-icons">keyboard_arrow_right</i> '.$navArticle->getValue('name').'</a></li>';
                            
                            if($articleUrl == ''){
                                $single_scroll_sub_list .= '<li><a class="waves-effect" href="'.$articleUrl.'#'.$navArticleId.'">'.$i.'</a></li>'; 
                                $single_scroll_sub_list_2 .= '<li><a class="waves-effect tooltipped" data-position="right" data-tooltip="'.$navArticle->getValue('name').'" href="'.$articleUrl.'#'.$navArticleId.'"><div>'.$i.'</div></a></li>';
                            } 
                        }
                        $i++;
                    }
                    ...
                    $side_nav .= '<li><a '.$activeClass.'href="'.$lev2->getUrl().'">'.$lev2->getName().'</a>'.$side_nav_sub.'</li>';  
                }else{
                    $side_nav .= '<li><a '.$activeClass.'href="'.$lev2->getUrl().'">'.$lev2->getName().'</a></li>'; 
                }   
            }    
        }
